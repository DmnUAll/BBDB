...
extension DetailedInfoController {
    ...
    func addWebButton(withLink link: String) {
        ...
        let webButton = UIBarButtonItem(image: UIImage(named: K.IconsNames.network)?.resize(targetSize: CGSize(width: iconSize, height: iconSize)), style: .plain, target: self, action: #selector(webButtonTapped))
    ...
    func fillUI(with data: Codable) {
        ...
        if let data = data as? EpisodeModel {
            detailedInfoView.infoStackView.addArrangedSubview(makeImageView(withImage: Bundle.main.url(forResource: K.ImagesNames.noImage, withExtension: "png")!))
            detailedInfoView.infoStackView.addArrangedSubview(labelStack)
    ...
    func fillUI(with data: NSManagedObject) {
        ...
        if let data = data as? CDCharacter {
            detailedInfoView.infoStackView.addArrangedSubview(makeImageView(withImage: data.imageURL ?? Bundle.main.url(forResource: K.ImagesNames.noImage, withExtension: "png")!))
            detailedInfoView.infoStackView.addArrangedSubview(labelStack)
